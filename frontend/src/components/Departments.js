...
export const Departments = () => {
  const crud = useCRUD('departments', departmentSchema);
    ...
    <div className="space-y-6">
      ...
      <Dialog open={crud.isDialogOpen} onOpenChange={crud.setIsDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{crud.editingItem ? 'Edit Department' : 'Create Department'}</DialogTitle>
            <DialogDescription>
              {crud.editingItem ? 'Update department details' : 'Add a new department to the system'}
            </DialogDescription>
          </DialogHeader>
